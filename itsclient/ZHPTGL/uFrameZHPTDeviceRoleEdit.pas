...
begin
  ...
  if TRequestItf.DbQuery('GetZHPTDeviceRole', Param) <> '' then
  begin
  ...
  begin
    Param := 'JSMC=' + Trim(edtRole.Text) + '&gxsj=' +
      ...
      '&bz=' + Trim(Memobz.Text);
    TRequestItf.DbQuery('ModifyZHPTDeviceRole', Param);
  end
  else
  begin
    ...
    if TRequestItf.DbQuery('AddZHPTDeviceRole', Param) <> '1' then
    begin
