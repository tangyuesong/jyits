...
{ TcxMonthListBox - Bug in Delphi5: Must be in the interface part }
  ...
  public
    ...
    property Animation: Boolean read FAnimation
      write SetAnimation default True;
...
begin
  if AFont.Size <> 0 then
    Result := Abs(AFont.Size)
  else
    Result := MulDiv(cxTextHeight(AFont), 72, AFont.PixelsPerInch);
end;
...
begin
  ...
  begin
    ASkinPainter.DrawHeader(ACanvas, Bounds, cxEmptyRect, [],
...
begin
  FTimeEditInfo := TcxCalendarTimeEditViewInfo.Create(Calendar);
  FClockInfo := TcxCalendarClockViewInfo.Create(Calendar);
end;
...
procedure TcxCalendarModernTimeViewInfo.CreateElements;
begin
  FTimeEditInfo := TcxCalendarTimeEditViewInfo.Create(Calendar);
  FClockInfo := TcxCalendarModernClockViewInfo.Create(Calendar);
end;
...
begin
  ...
  try
    ...
    dxAdjustToTouchableSize(FColWidth);
    FRowHeight := cxScreenCanvas.TextHeight('0') + 2;
    dxAdjustToTouchableSize(FRowHeight);
    ...
    dxAdjustToTouchableSize(FButtonsHeight);
  finally
    cxScreenCanvas.Dormant;
  end;
...
procedure TcxCalendarTouchUIViewInfo.CalculateBounds;
begin
  inherited CalculateBounds;
...
begin
  Result := cxNullSize;
  Result := cxSize(FDateTimeWheelPicker.Width, FDateTimeWheelPicker.Height);
  if IsButtonsVisible then
  begin
...
procedure TcxCalendarTouchUIViewInfo.DoSetSelectDate(AValue: Double; ARepositionVisibleDates: Boolean;
  AAnimated: Boolean);
begin
  Calendar.InternalSetSelectDate(AValue);
end;
...
begin
  ...
  begin
    ...
    FDateTimeWheelPicker.StyleFocused.BorderStyle := ebsNone;
    FDateTimeWheelPicker.StyleHot.BorderStyle := ebsNone;
  end
  else
  begin
    FDateTimeWheelPicker.Style.Edges := [bBottom];
    FDateTimeWheelPicker.StyleFocused.Edges := [bBottom];
    FDateTimeWheelPicker.StyleHot.Edges := [bBottom];
  end;
...
procedure TcxCalendarTouchUIViewInfo.SetFirstDate(Value: Double;
  AAnimated: Boolean);
begin
// do nothing
end;
...
procedure TcxCalendarPainter.DrawDateCellText(ACanvas: TcxCanvas;
  AViewInfo: TcxCalendarDateCellViewInfo);
begin
  cxDrawText(ACanvas, AViewInfo.GetText, AViewInfo.TextRect,
    DT_WORDBREAK or DT_EDITCONTROL or SystemAlignmentsHorz[AViewInfo.HorzAlignment]);
end;
...
procedure TcxCalendarModernPainter.DrawArrow(ACanvas: TcxCanvas;
  AViewInfo: TcxCalendarArrowViewInfo);
begin
  LookAndFeelPainter.DrawModernCalendarArrow(ACanvas, AViewInfo.Bounds, AViewInfo.Direction, AViewInfo.State);
end;
