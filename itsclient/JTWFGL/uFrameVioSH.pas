...
procedure TFrameVioSH.AfterConstruction;
begin
  inherited;
...
procedure TFrameVioSH.BeforeDestruction;
begin
  inherited;
  ClearVioList;
  FVioThread.Stop;
...
procedure TFrameVioSH.ClearQueue;
var
  cmd: PCmd;
begin
  cmd := FcmdQueue.Pop;
...
procedure TFrameVioSH.ClearVioList;
var
  vio: TVio;
begin
...
procedure TFrameVioSH.InitControl;
begin
  cbbwfdd.Text := '';
  cbbwfxw.Text := '';
  cbbsf.Text := '';
...
procedure TFrameVioSH.InitDevTb;
begin
  fSelectDev.tb.Close;
...
procedure TFrameVioSH.btnSaveClick(Sender: TObject);
var
  tmpWfdd: String;
begin
  FWfdd := '';
...
procedure TFrameVioSH.btnVideoClick(Sender: TObject);
begin
  if FObj = nil then
    exit;
...
procedure TFrameVioSH.LoadVioList;
var
  Param, s: String;
  vio: TVio;
begin
...
procedure TFrameVioSH.ObjDoned;
begin
  FObj.Doned := True;
...
procedure TFrameVioSH.FillControl;
var
...
procedure TFrameVioSH.FreeFrameWait;
begin
  if Assigned(FFrameWait) then
    FFrameWait.Visible := False;
end;

procedure TFrameVioSH.imgviopicMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  rc: TRect;
  ww, hh: Integer;
...
procedure TFrameVioSH.imgviopicMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFrameVioSH.LoadVehInfo(veh: TVehInfo);
var
...
procedure TFrameVioSH.UpdateColor(veh: TVehInfo);
var
  defaultColor: TColor;
...
procedure TFrameVioSH.UpdateDBVioStatus(systemid, bj: String; bz: String = '');
var
  Param: String;
  cmd: PCmd;
begin
  Param := 'SYSTEMID=' + systemid + '&ZT=' + bj + '&XGR=' + gUser.YHBH +
...
procedure TFrameVioSH.btnExitClick(Sender: TObject);
begin
...
procedure TFrameVioSH.btnLoadClick(Sender: TObject);
var
  json: String;
begin
  ShowFrameWait;
  ...
  InitDevTb;
  Application.ProcessMessages;
  json := TRequestItf.DbQuery('GetHisVioListTotalByFH', 'zt=29');
  if json = '' then
  begin
    FreeFrameWait;
    exit;
...
procedure TFrameVioSH.btnP1Click(Sender: TObject);
begin
  if FObj = nil then
    exit;
...
procedure TFrameVioSH.btnP2Click(Sender: TObject);
begin
  if FObj = nil then
    exit;
...
procedure TFrameVioSH.btnP3Click(Sender: TObject);
begin
  if FObj = nil then
    exit;
...
procedure TFrameVioSH.UpdateVehFromEdit();
var
  Key: String;
begin
  if FObj = nil then
...
procedure TFrameVioSH.btnBlackClick(Sender: TObject);
var
  Param, bklx: string;
  cmd: PCmd;
begin
...
procedure TFrameVioSH.btnDeleteClick(Sender: TObject);
begin
  if FObj = nil then
    exit;
...
procedure TFrameVioSH.cxButton2Click(Sender: TObject);
begin
  if FObj = nil then
    exit;
  {
...
procedure TFrameVioSH.dxLayoutGroup8Collapsed(Sender: TObject);
begin
  img1.Height := 260;
end;

procedure TFrameVioSH.dxLayoutGroup8Expanded(Sender: TObject);
begin
  img1.Height := 180;
end;

procedure TFrameVioSH.cbbsfKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    edthphm.SetFocus;
end;

procedure TFrameVioSH.cbbsfMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  cbbsf.SelectAll;
end;

procedure TFrameVioSH.cbbwfxwMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  cbbwfxw.SelectAll;
end;

procedure TFrameVioSH.cbbhpzlKeyPress(Sender: TObject; var Key: Char);
var
  veh: TVehInfo;
begin
  if Key = #13 then
...
procedure TFrameVioSH.cbbhpzlMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  cbbhpzl.SelectAll;
end;

procedure TFrameVioSH.edthphmKeyPress(Sender: TObject; var Key: Char);
var
  veh: TVehInfo;
begin
  if Key = #13 then
...
procedure TFrameVioSH.fBzbtnExitClick(Sender: TObject);
begin
  FOK := False;
  fBz.Visible := False;
...
procedure TFrameVioSH.fBzbtnSaveClick(Sender: TObject);
begin
  FOK := True;
  fBz.Visible := False;
  dxLayoutControl1Group_Root.Enabled := True;
...
procedure TFrameVioSH.ShowFrameWait;
begin
...
procedure TFrameVioSH.ShowVioPicture(picFile: string);
begin
  fz := False;
  FPicFile := picFile;
  imgviopic.AutoFit := True;
...
procedure TFrameVioSH.tbVioAfterDelete(DataSet: TDataSet);
begin
  tbVio.AfterScroll := tbVioAfterScroll;
end;

procedure TFrameVioSH.tbVioAfterScroll(DataSet: TDataSet);
var
  i: Integer;
begin
  FObj := nil;
...
procedure TFrameVioSH.tbVioBeforeDelete(DataSet: TDataSet);
begin
  tbVio.AfterScroll := nil;
end;

procedure TFrameVioSH.Timer1Timer(Sender: TObject);
var
  cmd: PCmd;
begin
  Timer1.Enabled := False;
...
procedure TFrameVioSH.Timer2Timer(Sender: TObject);
begin
