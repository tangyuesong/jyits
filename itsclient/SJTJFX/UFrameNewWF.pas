...
var
  FrameNewWF: TFrameNewWF;

implementation

{$R *.dfm}

procedure TFrameNewWF.AfterConstruction;
begin
  inherited;
  RDMonth.Checked := True;
  RDDate.Checked := false;
  GridColumns := 'WFXWDM,WFXW,当月,上月,上年同月,环比增长(%),同比增长(%)';

end;

procedure TFrameNewWF.BtnSearchClick(Sender: TObject);
var
  s: string;
  y, m, period: string;
begin
  ...
  begin
    ...
    period := 'y';
    Jkid := 'getSurSL2';
    GridColumns := 'WFXWDM,WFXW,当年,上年，环比增长(%)';
  ...
  FDMemTable1.FieldDefs.Add('WFXWDM', ftString, 50, false);
  FDMemTable1.FieldDefs.Add('WFXW', ftString, 50, false);
