...
begin
  ...
  for i := 0 to files.Count - self.MaxBackupIndex do
  begin
  ...
  begin
    ...
    n := strtointdef(copy(s, 2, length(s)), 0);
    if (n = 0) and (fullFilename <> FLogFileName) then
      break;
  ...
  FLogStream := TFileStream.Create(FLogFileName, fmOpenReadWrite or
    fmShareDenyWrite);
end;
...
begin
  ...
  FLogStream := TFileStream.Create(ALogFile, fmOpenReadWrite or
    fmShareDenyWrite);
...
begin
  ...
  sThreadId := ' [' + sThreadId + '] ';
  s := formatdatetime('yy-mm-dd hh:mm:ss', now) + sThreadId + sLevel +
    AMsg + #13#10;
