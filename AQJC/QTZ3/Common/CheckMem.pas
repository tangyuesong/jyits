...
begin
  ...
  try
    ...
    with HeapStatus do
    begin
      Writeln(OutFile, '===== ', ExtractFileName(ParamStr(0)), ',',
        DateTimeToStr(Now), ' =====');
      Writeln(OutFile);
      ...
      Writeln(OutFile, '地址空间载入 : ':FIELD_WIDTH,
    ...
    begin
      ...
      try
        ...
        if PTypeInfo(Item.ClassInfo).Kind <> tkClass
        then { type info technique }
          write(OutFile, '不是对象')
        else
        begin
...
const
  NewMemMgr: TMemoryManager = (GetMem: NewGetMem; FreeMem: NewFreeMem;
...
if (GetMemCount - FreeMemCount) <> 0 then
  SnapCurrMemStatToFile(ExtractFilePath(ParamStr(0)) + 'CheckMemory.Log');
