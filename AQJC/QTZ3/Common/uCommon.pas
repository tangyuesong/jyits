...
uses
  ...
  StrUtils, System.NetEncoding, System.Math, NB30;
...
type
  ...
  private
    ...
    class var FPolicySetting: TPolicySetting;
    ...
    class procedure GetPolicySetting;
  public
    ...
    class property PolicySetting: TPolicySetting read FPolicySetting;
...
class function TCommon.GetMACAddress: string;
var
...
class procedure TCommon.GetPolicySetting;
begin
  FPolicySetting.LoginErrorUser := 5;
  FPolicySetting.LoginErrorTerminal := 5;
  FPolicySetting.LockTime := 60;
  FPolicySetting.RequestCount := 60;
  FPolicySetting.UnUseDays := 30;
  with gSQLHelper.Query('select * from ' + cDBName + '.dbo.S_Policy_Setting') do
  begin
    while not Eof do
    begin
      if FieldByName('key').AsString = 'LoginErrorUser' then
        FPolicySetting.LoginErrorUser := FieldByName('value').AsInteger
      else if FieldByName('key').AsString = 'LoginErrorTerminal' then
        FPolicySetting.LoginErrorTerminal := FieldByName('value').AsInteger
      else if FieldByName('key').AsString = 'LockTime' then
        FPolicySetting.LockTime := FieldByName('value').AsInteger
      else if FieldByName('key').AsString = 'RequestCount' then
        FPolicySetting.RequestCount := FieldByName('value').AsInteger
      else if FieldByName('key').AsString = 'UnUseDays' then
        FPolicySetting.UnUseDays := FieldByName('value').AsInteger;
      Next;
    end;
    Free;
  end;
