...
uses
  ...
  IdStrings, QBAES, uLoginUtils,
  FireDAC.Phys.OracleDef, FireDAC.Stan.Intf, FireDAC.Phys, FireDAC.Phys.Oracle,
  IdTime, IdUnixTime, IdServerIOHandler, IdSSL, IdSSLOpenSSL, NB30;
...
begin
  ...
  s := httpClient.Get
    (TIdURI.URLEncode
    ('https://127.0.0.1:37115/Login?user=su&pwd=EB8E560662808ED4FAEA1C8AF016055947C4EF17CFD7AFC5390AA126AB25E2A2&id='
    + TCommon.GetMACAddress)
    // ('https://127.0.0.1:37115/Login?user=250001&pwd=D3EAFA3C63FC6A7D1DFAADD0203776A1&id='+TCommon.GetMACAddress)
  ...
  // (TIdURI.URLEncode('http://10.43.255.8:17115/GetVioinfoByVeh?hphm=粤YHU567&hpzl=02&token='
    (TIdURI.URLEncode('https://127.0.0.1:37115/ModifyS_User?systemid=1111&zt=1&'
...
begin
  ...
  begin
    ...
    if params.Values['zt'] = '1' then
      TLoginManager.SaveSecurityLog('1', '用户解锁', '1', yhbh, clientIP,
        params.Text);
  end;
