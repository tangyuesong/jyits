...
uses
  ...
  dxSkinXmas2008Blue, dxSkinscxPCPainter, cxCheckBox, uPicView,
...
begin
  inherited;
  if not Assigned(FPicView) then
  begin
    FPicView := TPicView.Create(self);
    FPicView.Parent := self;
    FPicView.Align := TAlign.alClient;
  end;
  FPicView.GCTable := FDMemTable1;
  FPicView.Show;
end;
...
begin
  ...
  begin
    params := 'passtime=' + FormatDateTime('yyyy-mm-dd hh:mm:ss', dtBegin.Date)
      ...
      pageSize.ToString;
    if FKDBH <> '' then
      params := params + '&crossingid=' + TLZDictionary.gDicDev[2][FKDBH].HikID;
    if FHPZL <> '' then
    begin
      hpzls := '';
