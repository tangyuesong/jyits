...
procedure TFramePGS.setCenter(lng, lat: Double; zoom: Integer);
var
  script: WideString;
begin
...
procedure TFramePGS.Timer1Timer(Sender: TObject);
begin
  inherited;
...
procedure TFramePGS.Timer2Timer(Sender: TObject);
var
  lng, lat: Double;
begin
  inherited;
...
procedure TFramePGS.cboDevPropertiesCloseUp(Sender: TObject);
var
  SBBH: string;
  device: TDevice;
  lng, lat: Double;
...
procedure TFramePGS.cboSBLXPropertiesChange(Sender: TObject);
begin
  inherited;
  LoadDevice;
end;

procedure TFramePGS.LoadDevice;
var
  s, SBLX: string;
  list: TList<TDevice>;
  dev: TDevice;
...
begin
  ...
  begin
    ...
    if (point.lng > 0) and (point.lng < 180) and (point.lat > 0) and (point.lat < 90) then
    begin
      point := point.WGS84ToGCJ02.GCJ02ToBD09;

      js.Add(Format('addMarker(%.6f, %.6f,"%s","%s","%s");',
        [point.lng, point.lat, 'camera-blue.png', 'QDZ_'+dev.QDZMC, dev.QDZDM]));
    end;
  end;
  FJsExtended.ExecuteJavaScript(js.Text);
  js.Free;
