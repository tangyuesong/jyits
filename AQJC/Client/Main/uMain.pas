...
{$ENDIF}
  ...
  uCommon, uFrameIndexGCTJ, UFrameTJJKCJ, cxTextEdit, cxMemo, uModifyPwd,
  uSelfInfo;
...
type
  ...
  private
    ...
    Procedure ModifyPwd(Sender: TObject);
    Procedure LookSelfInfo(Sender: TObject);
  public
...
{$ENDIF}
  ...
  barGroup.Expanded := False;
  barGroup.ShowExpandButton := False;
  barGroup.SmallImageIndex := 28;
  barGroup.Caption := '个人相关';
  barGroup.OnExpanded := mbMainGroupExpanded;

  barItem := mbMain.Items.Add;
  barItem.SmallImageIndex := 33;
  barItem.Caption := '修改登录密码';
  barItem.OnClick := ModifyPwd;
  barGroup.CreateLink(barItem);

  barItem := mbMain.Items.Add;
  barItem.SmallImageIndex := 34;
  barItem.Caption := '个人相关信息';
  barItem.OnClick := LookSelfInfo;
  barGroup.CreateLink(barItem);
...
procedure TfrmMain.ModifyPwd(Sender: TObject);
begin
...
procedure TfrmMain.LookSelfInfo(Sender: TObject);
begin
  Application.CreateForm(TfrmSelfInfo, frmSelfInfo);
  frmSelfInfo.ShowModal;
  FreeAndNil(frmSelfInfo);
end;
