...
uses
  ...
  Vcl.Imaging.jpeg, EncdDecd, sfContnrs, uFrameVioCopy,
...
begin
  ...
  if veh.zt.Contains('M') or veh.zt.Contains('P')  then
    edtqzbfqz.Style.TextColor := clRed;
...
procedure TFrameVioInput.btnCopyClick(Sender: TObject);
begin
  if not Assigned(FFrameVioCopy) then
  begin
    FFrameVioCopy := TFrameVioCopy.Create(self);
    FFrameVioCopy.Parent := self;
    FFrameVioCopy.Align := alClient;
  end;
  FFrameVioCopy.VioObj := FObj;
  FFrameVioCopy.Show;
end;
