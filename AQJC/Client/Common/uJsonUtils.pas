...
type
  ...
  public
    Class function JSONToDataSet(AJSON: String; ATable: TFDMemTable;
      ...
      Options: TIndexOptions = [ixDescending];
      IsEncrypt: Boolean = False): Boolean;
...
begin
  ...
  s := TCommon.GetJsonNode('body', JSON);
  if s <> '' then
    JSON := s;
  if JSON = '' then
    exit;
...
begin
  ...
  if JSON = '' then
    exit;
  if (copy(JSON, 1, 1) <> '[') and (copy(JSON, length(JSON), 1) <> ']') then
    JSON := '[' + JSON + ']';
...
Class function TJSONUtils.JSONToDataSet(AJSON: String; ATable: TFDMemTable;
  AIndexField: String = ''; AIsCreateCol: Boolean = True;
  Options: TIndexOptions = [ixDescending]; IsEncrypt: Boolean = False): Boolean;
var
  ja: TJSONArray;
  jv: TJSONValue;
  ji: TJSONValue;
...
begin
  ...
  if ATable = nil then
    exit;
  AJSON := TCommon.GetJsonNode('body', AJSON);
  ATable.Close;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if IsEncrypt then
              ATable.FieldByName(colName).AsString :=
                AesDecrypt(TJSONPair(ji).JsonValue.Value, key)
            else
