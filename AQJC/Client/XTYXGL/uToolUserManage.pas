...
begin
  ...
  GridColumns := 'DWDM, YHBH, YHXM, SJHM, FH, Manager, LRR,ZT,操作';
...
begin
  ...
  if not FDMemTable1.Active or FDMemTable1.Eof then
    exit;
  if FDMemTable1.FieldByName('zt').AsBoolean then
  begin
    Application.MessageBox('该用户是激活状态，无须解锁', '提示', MB_OK + MB_ICONINFORMATION);
    exit;
  end;

  s := 'systemid=' + FDMemTable1.FieldByName('systemid').AsString + '&zt=1';
  s := TRequestItf.DbQuery('ModifyS_User', s);
  if TCommon.GetJsonNode('code', s) <> '1' then
    Application.MessageBox('解锁失败', '提示', MB_OK)
  else
  begin
    Application.MessageBox('解锁成功', '提示', MB_OK);
    FDMemTable1.Edit;
    FDMemTable1.FieldByName('zt').AsString := '1';
    FDMemTable1.Post;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      TRequestItf.DbQuery('DelS_User', 'SYSTEMID=' + id);
      TRequestItf.DbQuery('DelS_ROLE_USER', 'yhbh=' + id);
      TRequestItf.DbQuery('DelS_DataROLE_USER', 'yhbh=' + id);
      FDMemTable1.Delete;
    end;
  end;
end;
...
begin
  ...
  if not gIsSa then
    Param := Param + '&dwdm=' + TCommon.GetXZQH(gUser.dwdm);
  if Trim(edtDwdm.Text) <> '' then
    Param := Param + '&dwdm=' + Trim(edtDwdm.Text);
