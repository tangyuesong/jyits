...
uses
  ...
  Generics.Collections, uGlobal, uCommon;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not gIsSa and
          (gUserPower.UserFunction[cm.SystemID].IndexOf(af.ControlName) < 0)
        then
          continue;
...
begin
  ...
  s := TRequestItf.DbQuery('GetS_Role_List', param);
  s := TCommon.GetJsonNode('totalnum', s);
  ...
  begin
    ...
    if Trim(Memobz.Text) <> '' then
      param := param + '&bz=' + Trim(Memobz.Text);
    s := TRequestItf.DbQuery('AddS_Role_List', param);
